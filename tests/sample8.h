...
void test_file_output(void)noexcept{
    ...
    if(ofs.is_open()){
        ...
        ofs<<::vnitest::execute_group_to_json("passed_group")<<'\n';
        ofs<<::vnitest::execute_group_to_json("failed_group")<<'\n';
        ofs<<::std::format("execute group(group list) {}\n",line);
        ofs<<::vnitest::execute_group_to_json({"passed_group","failed_group"})<<'\n';
        ofs.close();
    }else{
